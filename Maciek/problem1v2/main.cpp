...
int main()
{
	...
	cin >> inFileName;//plik komend
	inFile.open(inFileName.c_str());


	cout << "Enter name of output file; press return." << endl;
	cin >> outFileName;
	outFile.open(outFileName.c_str());//plik danych wyjsciowych


	cout << "Enter name of input data file; press return." << endl;
	cin >> inDataName;
	inData.open(inDataName.c_str());//plik danych wejsciowych
	

	cout << "Enter name of test run; press return." << endl;
	cin >> outputLabel;
	outFile << outputLabel << endl;//nazwa testu


	inFile >> command;
	numCommands = 0;
	while (command != "Quit")//obsluga komend
	{
...
InType Allowed(std::string& inString)//zwraca InType
{
...
bool Skip(std::string& inString)//zwraca skip
{
	if (inString == "true")
		return true;
	else return false;
